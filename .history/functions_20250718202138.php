...
require_once get_template_directory() . '/inc/class-asset-controller.php';
...
// Debug function to check registered blocks
function debug_registered_blocks() {
    if (WP_DEBUG) {
        ...
        foreach ($our_blocks as $name => $block) {
            if (isset($block->category)) {
                error_log("Block $name has category: " . $block->category);
            }
        }
    }
}
...
function tailwind_starter_admin_notice() {
    if (WP_DEBUG && current_user_can('manage_options')) {
        $controller = get_asset_controller();
        if ($controller) {
            $blocks = $controller->get_registered_blocks();
            $count = count($blocks);
            $categories = $controller->get_used_categories();
            
            echo '<div class="notice notice-info is-dismissible">';
            echo '<p><strong>Tailwind Starter Debug:</strong></p>';
            echo '<p>Registered ' . $count . ' blocks in categories: ' . implode(', ', $categories) . '</p>';
            echo '<p>To see blocks, go to Posts > Add New and click the "+" button to open the block inserter.</p>';
            echo '</div>';
        }
    }
}
add_action('admin_notices', 'tailwind_starter_admin_notice');
...
function my_tailwind_starter_block_categories($categories) {
    ...
    if (WP_DEBUG) {
        error_log('Block categories registered: ' . json_encode(array_column($custom_categories, 'slug')));
    }
